...
import me.abanda.infrastructure.{CorrelationId, DB, Doobie, SetCorrelationIdBackend}
import me.abanda.metrics.Metrics
import me.abanda.util.DefaultClock
...
object Main extends ResourceApp.Forever with StrictLogging {
  ...
  val sttpBackend: Resource[IO, SttpBackend[IO, Fs2Streams[IO] with WebSockets]] =
    AsyncHttpClientFs2Backend
      ...
      .map(baseSttpBackend =>
        Slf4jLoggingBackend(
          PrometheusBackend(new SetCorrelationIdBackend(baseSttpBackend)),
          includeTiming = true
        )
      )
  ...
  val xa: Resource[IO, Doobie.Transactor[IO]] =
    new DB(config.db).transactorResource.map(CorrelationId.correlationIdTransactor)
   ...
   *
   * Thanks to ResourceApp.Forever the result of the allocation is used by a non-terminating process
   * (so that the http server is available as long as our application runs).
   */
